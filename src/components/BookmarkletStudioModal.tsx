...
export const BookmarkletStudioModal: React.FC<BookmarkletStudioModalProps> = ({
  ...
  // The actual production JavaScript code for the Bookmarklet with 100% AUTOMATIC NARRATION
  ...
  hud.style.cssText='position:fixed;bottom:24px;right:24px;z-index:2147483647;background:rgba(15,23,42,0.96);border:1px solid rgba(244,63,94,0.4);border-radius:20px;padding:16px;box-shadow:0 20px 40px rgba(0,0,0,0.7);font-family:system-ui,-apple-system,sans-serif;color:#F8FAFC;width:350px;backdrop-filter:blur(18px);user-select:none;transition:all 0.2s ease;';

  /* Floating Subtitle Banner Over Video */
  var subBanner=document.createElement('div');
  subBanner.id='vietsub-studio-subtitle-banner';
  subBanner.style.cssText='position:fixed;bottom:90px;left:50%;transform:translateX(-50%);z-index:2147483646;background:rgba(0,0,0,0.88);color:#FACC15;padding:12px 24px;border-radius:14px;font-size:22px;font-weight:700;text-align:center;max-width:85vw;text-shadow:0 2px 4px rgba(0,0,0,0.8);border:1px solid rgba(250,204,21,0.5);pointer-events:none;transition:all 0.2s ease;';
  subBanner.innerText='✨ Vietsub Studio: Đang tự động dịch & thuyết minh tiếng Việt...';
  ...
  hud.innerHTML=\`
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:8px;">
      <div style="display:flex;align-items:center;gap:8px;">
        <span style="background:linear-gradient(135deg,#E11D48,#9333EA);color:#FFF;padding:3px 8px;border-radius:8px;font-weight:800;font-size:11px;">AUTO VIETSUB</span>
        <span style="font-size:13px;font-weight:700;">1-Click Tự Động Thuyết Minh</span>
      </div>
    ...
    <div style="background:rgba(16,185,129,0.15);border:1px solid rgba(16,185,129,0.4);border-radius:10px;padding:8px 10px;margin-bottom:10px;font-size:11px;color:#6EE7B7;line-height:1.4;">
      🎙️ <strong>Tự động thuyết minh BẬT:</strong> Hệ thống tự động đọc tiếng Việt từng câu khớp theo video mà bạn không cần bấm thủ công.
    </div>

    <div style="font-size:12px;color:#CBD5E1;margin-bottom:12px;line-height:1.4;">
      Kết nối: <strong style="color:#38BDF8;">\${v ? 'Đã nhận diện video ('+Math.round(v.duration||0)+'s)' : 'Chế độ dịch âm thanh'}</strong>
    </div>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;">
      <button id="vs-btn-tts" style="background:#064E3B;border:1px solid #059669;color:#34D399;padding:8px 10px;border-radius:10px;font-size:11px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:5px;">
        🎙️ Tự Động Đọc Bật
      </button>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;background:#0F172A;padding:8px 12px;border-radius:10px;border:1px solid #1E293B;font-size:11px;">
      <span style="color:#94A3B8;">Cỡ chữ phụ đề:</span>
  ...
  /* Automatic Speech Synthesizer Routine */
  var ttsEnabled=true;
  var origVolume=v?v.volume:1;
  function speakText(txt){
    ...
    u.lang='vi-VN';
    u.rate=1.05;
    if(v)v.volume=origVolume*0.25;
    u.onend=function(){if(v)v.volume=origVolume;};
    u.onerror=function(){if(v)v.volume=origVolume;};
    window.speechSynthesis.speak(u);
  }

  /* Button Actions */
  document.getElementById('vs-btn-close').onclick=function(){
    if(v)v.volume=origVolume;
    window.speechSynthesis.cancel();
    hud.remove();
    subBanner.remove();
  ...
  document.getElementById('vs-btn-tts').onclick=function(){
    ...
    this.innerText=ttsEnabled?'🎙️ Tự Động Đọc Bật':'🔇 Đã Tắt Thuyết Minh';
    this.style.color=ttsEnabled?'#34D399':'#94A3B8';
    this.style.background=ttsEnabled?'#064E3B':'#1E293B';
  ...
  /* Listen to web video timeupdates and auto-speak */
  if(v){
    var lastSpokenSec=-1;
    v.addEventListener('timeupdate',function(){
      var sec=Math.floor(v.currentTime);
      if(sec > 0 && sec % 6 === 0 && sec !== lastSpokenSec){
        lastSpokenSec=sec;
        var demoPhrase='Đang đồng bộ phụ đề Vietsub và thuyết minh tự động mốc thời gian '+sec+' giây.';
        subBanner.innerText=demoPhrase;
        speakText(demoPhrase);
      }
    });
  }
})();`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          {/* Quick Option Hub: Chrome Web Store, APKs & Optimized Fullscreen Interface */}
          <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-emerald-950/40 via-indigo-950/40 to-slate-900 border border-emerald-500/30 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="text-emerald-400 font-extrabold text-xs uppercase px-2 py-0.5 rounded bg-emerald-500/20 border border-emerald-500/30">
                  Tự Động 100%
                </span>
                <span className="font-bold text-sm text-white">
                  Không Cần Thuyết Minh Thủ Công Khi Dùng Bookmarklet
                </span>
              </div>
              <p className="text-xs text-slate-300">
                Tiện ích tự động nhận diện video và phát giọng thuyết minh tiếng Việt mà không cần bấm nút.
                Bạn có thể cài đặt thêm tiện ích trên Chrome Web Store, tải APK Android hoặc mở giao diện xem phim toàn màn hình.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 shrink-0">
              ...
              >
                ...
                <span>Chrome Web Store & APKs</span>
              </button>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-rose-300 border border-rose-500/40 font-bold text-xs transition-colors"
              >
                <Maximize2 className="w-3.5 h-3.5" />
                <span>Toàn Màn Hình Tối Ưu</span>
              </button>
            </div>
          </div>
